...
class LIBRIICBHIDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        ...
        for index in range(len(self.audio_data)): #for the training set, 4142
            ...
            for aug_idx in range(self.args.raw_augment+1):
                if aug_idx > 0:
                    ...
                    if args.model in ['hubert_base', 'hubert_large_ll60k', 'wav2vec2_base_960', 'wav2vec2_large_ll60k', 'xls_r_300m', 'xls_r_1b', 'xls_r_2b', 
        ...
        'facebook/wav2vec2-large-lv60', 'facebook/wav2vec2-xls-r-300m', 'facebook/wav2vec2-xls-r-1b']:
                        ...
                        image = generate_fbank(args, audio, self.sample_rate, n_mels=self.n_mels)
                    audio_image.append(image)
            self.audio_images.append((audio_image, torch.tensor(0), meta_label))
                        
        print(len(self.audio_images))
        
        libridata = sorted(glob('./data/LibriSpeech/test-clean/*/*/*.flac'))
        
        print(len(libridata))
        
        for index in libridata:
            #print('index', index)
            audio_image = []
            audio = self.get_individual_cycles_torchaudio(index)
            #print('output audio', audio.size())
            audio = augment_raw_audio(np.asarray(audio.squeeze(0)), self.sample_rate, self.args)
            audio = cut_pad_sample_torchaudio(torch.tensor(audio), self.args)
            #print('output audio3', audio.size())
            self.audio_images.append((audio, torch.tensor(1), None))
    
    def get_individual_cycles_torchaudio(self, wav_samples):
        data, sr = torchaudio.load(wav_samples)
        ...
        data = cut_pad_sample_torchaudio(data, self.args) # --> resample to [1, 128000] --> 8 seconds
...
class AudioSetICBHIDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        data_folder = os.path.join(args.data_folder, 'icbhi_dataset/audio_test_data')
        folds_file = os.path.join(args.data_folder, 'icbhi_dataset/patient_list_foldwise.txt')
        official_folds_file = os.path.join(args.data_folder, 'icbhi_dataset/official_split.txt')
        ...
        #audiodata = sorted(glob('./data/LibriSpeech/test-clean/*/*/*.flac'))
        audiodata = sorted(glob('./data/audioset/eval_segments/*.wav'))
        print(len(audiodata))
        
        #audiodata = audiodata[:1000]
        ...
        for index in audiodata:
            ...
            if args.model == 'ast':
                ...
                self.audio_images.append((image, torch.tensor(1), None))
            else:
                self.audio_images.append((audio, torch.tensor(1), None))
