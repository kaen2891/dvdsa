...
def get_samples_torchaudio(data, sample_rate):
...
def get_individual_samples_torchaudio(args, folder_name, data_folder, sample_rate, n_cls, train_flag, label, cesdc, scared):
    sample_data = []
    ...
    for data in subject_files:
        ...
        if train_flag:
            ...
            for i in range(divide_count):
                ...
                sample_data.append((divided_wav, label, cesdc, scared))
        else:
            sample_data.append((data, label, cesdc, scared))
    
    padded_sample_data = []
    for data, label, cesdc, scared in sample_data:
        data = cut_pad_sample_torchaudio(data, train_flag, args) # --> resample to [1, 80000] --> 5 seconds
        padded_sample_data.append((data, label, cesdc, scared))
