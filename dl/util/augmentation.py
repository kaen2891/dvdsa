...
# Use this Class when you load dataset with librosa
class RepAugment(torch.nn.Module):
    print('RepAugment using')
    '''
    Unofficial Implementation of SpecAugment: A Simple Data Augmentation Method for Automatic Speech Recognition
    Paper: https://arxiv.org/pdf/1904.08779.pdf
    Ref. github: https://github.com/pyyush/SpecAugment/blob/219fc6e9ed4838fe9700295700040b1da283c536/augment.py#L10
    ...
    def __init__(self, args):
        ...
        self.policy = args.specaug_policy
        self.mask_value = args.specaug_mask
        ...
        ##Rep        
        elif self.policy == 'icbhi_cnns_rep':
            self.W, self.F, self.m_F, self.T, self.p, self.m_T = 0, 192, 2, 0, 1.0, 0
        elif self.policy == 'icbhi_ast_rep':
            self.W, self.F, self.m_F, self.T, self.p, self.m_T = 0, 288, 2, 0, 1.0, 0
    ...
    def freq_mask(self):
        if self.mask_value == 'mean':
            # maksing to mean value
            mask_value = self.representations.mean()
        elif self.mask_value == 'zero':
            # maksing to zero value
            mask_value = 0.

        v = self.mask.shape[1] # no. of mel bins
        
        # apply m_F frequency masks to the mel spectrogram
        for i in range(self.m_F):
            f = int(np.random.uniform(0, self.F)) # [0, F)
            f0 = random.randint(0, v - f) # [0, v - f)
            self.mask[:, f0:f0 + f, :] = mask_value
    ...
    def time_mask(self):
        ...
        tau = self.mask.shape[2] # time frames
        
        # apply m_T time masks to the mel spectrogram
        for i in range(self.m_T):
            t = int(np.random.uniform(0, self.T)) # [0, T)
            t0 = random.randint(0, tau - t) # [0, tau - t)
            self.mask[:, :, t0:t0 + t] = mask_value
    ...
    def forward(self, representation):
        ...
        self.mask = torch.ones_like(self.representations)
        #self.mask = torch.ones_like(self.representations).to(device=self.representation.device) # This is not working. 
        #self.mask = torch.ones_like(self.representations)
        #print('mask', self.mask, self.mask.size())
        if self.mask.dim() == 2:
           self.mask = self.mask.unsqueeze(1)
        
        self.mask = self.mask.transpose(2, 1) # torch.tensor [B, dim, time]
        #print('mask2', self.mask.size())
        #print('before repaug, mask', self.mask.size())
        ...
        if self.p >= torch.randn(1):
            if self.W:
                try:
                    # input shape of time_warp should be [sample_size, time, freq]
                    # assume that channel == 1 and augment each "one" sample
                    self.mask = self.time_warp()
                except Exception as e:
                    # torch.linalg.solve: (Batch element 0): The solver failed because the input matrix is singular.
                    # print(e)
                    pass

            self.mask = self.freq_mask()
            self.mask = self.time_mask()
        
        #if self.policy == 'icbhi_ast_rep':
        self.mask = self.mask.squeeze(-1)
        #print('output mask', self.mask)
        return self.mask
        '''
        if self.policy == 'icbhi_ast_rep':
            self.mask = self.mask.squeeze(-1)
            return self.mask
        return self.mask.transpose(2, 1).unsqueeze(1) # B, time, freq
        '''
